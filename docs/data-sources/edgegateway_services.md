...
- `edge_gateway_id` (String) Identifier of the Edge Gateway. Ensure that one and only one attribute from this collection is set : `edge_gateway_name`, `edge_gateway_id`. Must be a valid URN. This value must start with `urn:vcloud:gateway:`.
- `edge_gateway_name` (String) Name of the Edge Gateway. Ensure that one and only one attribute from this collection is set : `edge_gateway_name`, `edge_gateway_id`. Edge Gateway name (tn01e02ocb0001234spt101).
