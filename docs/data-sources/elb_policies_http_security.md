...
- `virtual_service_id` (String) The ID of the virtual service to which the policies http security belongs.
...
- `rate_limit` (Attributes) The rate_limit can be set an action when the rate limit is reached. A rate limit is a limit on the number of requests that can be made in a given time period. (see [below for nested schema](#nestedatt--policies--actions--rate_limit))
- `redirect_to_https` (Number) Redirect to HTTPS action.
...
- `content` (String) Content of the response must be a base64 encoded string. IE: result of `echo -n 'Hello World' | base64` will be `SGVsbG8gV29ybGQ=`.
