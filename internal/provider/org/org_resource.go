...
func (r *OrgResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	*/
	...
	if plan.InternetBillingMode != state.InternetBillingMode && !plan.InternetBillingMode.IsNull() {
	...
	stateRefreshed, diags := toOrgModel(ctx, orgUpdated)
	if diags.HasError() {
...
func (r *OrgResource) read(ctx context.Context) (stateRefreshed *OrgModel, diags diag.Diagnostics) {
	...
	stateRefreshed, diags = toOrgModel(ctx, org)
	if diags.HasError() {
		return nil, diags
	}
