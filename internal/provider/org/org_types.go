...
func toOrgModel(ctx context.Context, o *types.ModelGetOrganization) (data *OrgModel, diags diag.Diagnostics) {
	if o == nil {
	...
	x := toOrgModelResources(&o.Resources)

	diags.Append(data.Resources.Set(ctx, x)...)
	if diags.HasError() {
		return nil, diags
	}
...
func toOrgModelResources(r *types.ModelGetOrganizationResources) (data *OrgModelResources) {
	if r == nil {
		return nil
	}
	data = &OrgModelResources{}

	data.VDC.SetInt(r.Vdc)
	data.Catalog.SetInt(r.Catalog)
	data.Vapp.SetInt(r.Vapp)
	data.VMRunning.SetInt(r.VMRunning)
	data.User.SetInt(r.User)
	data.Disk.SetInt(r.Disk)
