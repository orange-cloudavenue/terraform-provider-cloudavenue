...
type groupFirewallModel struct {
	ID       supertypes.StringValue                                     `tfsdk:"id"`
	VDCGroup supertypes.StringValue                                     `tfsdk:"vdc_group"`
	Enabled  supertypes.BoolValue                                       `tfsdk:"enabled"`
	Rules    supertypes.ListNestedObjectValueOf[groupFirewallModelRule] `tfsdk:"rules"`
}

type groupFirewallModelRule struct {
	ID                  supertypes.StringValue        `tfsdk:"id"`
...
func (rm *groupFirewallModel) Copy() *groupFirewallModel {
	x := &groupFirewallModel{}
	utils.ModelCopy(rm, x)
	return x
}

func (rm *groupFirewallModel) rulesToSDKRules(ctx context.Context) (sdkrules v1.VDCGroupFirewallType, diags diag.Diagnostics) {
...
// sdkRulesToRules.
func (rm *groupFirewallModel) sdkRulesToRules(ctx context.Context, rules v1.VDCGroupFirewallTypeRules) (terraformRules []*groupFirewallModelRule, diags diag.Diagnostics) {
	terraformRules = make([]*groupFirewallModelRule, len(rules))
	for i, r := range rules {
		terraformRules[i] = &groupFirewallModelRule{
