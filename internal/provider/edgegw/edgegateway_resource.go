...
func (r *edgeGatewayResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error listing edge gateways", fmt.Sprintf("error listing edge gateways while creating edge gateway %s: %s", plan.Name.Get(), err.Error()))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error retrieving VDC or VDC Group", fmt.Sprintf("error retrieving VDC or VDC Group for edge gateway %s: %s", plan.Name.Get(), err.Error()))
		return
	}
	...
	if err := job.Wait(1, int(createTimeout.Seconds())); err != nil {
		resp.Diagnostics.AddError("Error waiting for edge gateway creation", fmt.Sprintf("error waiting for edge gateway %s creation: %s", plan.Name.Get(), err.Error()))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error listing edge gateways", fmt.Sprintf("error listing edge gateways after creating edge gateway %s: %s", plan.Name.Get(), err.Error()))
		return
	}
	...
	if edgegwNew == (v1.EdgeGatewayType{}) {
		resp.Diagnostics.AddError("Error retrieving new edge gateway", fmt.Sprintf("newly created edge gateway %s not found in refreshed edge gateway list", plan.Name.Get()))
		return
	}
	...
	if edgegwNew.GetBandwidth() != plan.Bandwidth.GetInt() {
		...
		if err != nil {
			resp.Diagnostics.AddError("Error updating bandwidth", fmt.Sprintf("error updating bandwidth for edge gateway %s during create: %s", plan.Name.Get(), err.Error()))
			return
		}

		if job != nil {
			if err := job.Wait(1, int(createTimeout.Seconds())); err != nil {
				resp.Diagnostics.AddError("Error waiting for bandwidth update", fmt.Sprintf("error waiting for bandwidth update on edge gateway %s during create: %s", plan.Name.Get(), err.Error()))
				return
			}
		}
	}
...
func (r *edgeGatewayResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if !plan.Bandwidth.Equal(state.Bandwidth) {
		...
		if err != nil {
			resp.Diagnostics.AddError("Error retrieving edge gateway", fmt.Sprintf("error retrieving edge gateway %s for update: %s", plan.Name.Get(), err.Error()))
			return
		}

		job, err := edgegw.UpdateBandwidth(plan.Bandwidth.GetInt())
		if err != nil {
			resp.Diagnostics.AddError("Error updating bandwidth", fmt.Sprintf("error updating bandwidth for edge gateway %s: %s", plan.Name.Get(), err.Error()))
			return
		}

		if err := job.Wait(1, int(updateTimeout.Seconds())); err != nil {
			resp.Diagnostics.AddError("Error waiting for bandwidth update", fmt.Sprintf("error waiting for bandwidth update on edge gateway %s: %s", plan.Name.Get(), err.Error()))
			return
		}
	}
...
func (r *edgeGatewayResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Error retrieving edge gateway", fmt.Sprintf("error retrieving edge gateway %s for deletion: %s", state.Name.Get(), err.Error()))
		return
	}
...
func (r *edgeGatewayResource) read(_ context.Context, planOrState *edgeGatewayResourceModel) (stateRefreshed *edgeGatewayResourceModel, found bool, diags diag.Diagnostics) {
	...
	if err != nil {
		...
		diags.AddError("Error retrieving edge gateway", fmt.Sprintf("error retrieving edge gateway %s during read: %s", planOrState.Name.Get(), err.Error()))
